...
# direction是邻接矩阵的索引，edgedirection是对应边缘的索引，edgeotherdirection是边缘的另一侧索引
def diretionDetect(adjacents, all_edges, positions, idx, direction, edgedirection, edgeotherdirection):
    for adjacent in adjacents[direction]:
        ...
        if adjacent > idx:
            ...
            # 当前图片反射图像存在上边缘
            if len(all_edges[idx][edgedirection]):
                # 上邻接图片反射图像存在下边缘
                if len(all_edges[adjacent][edgeotherdirection]):
                    # 两个邻接窗户的反射边缘相对横坐标
                    # 现在只考虑边缘只有一组范围的情况，如果测试中发现可能有多段范围，再进行修改！！！
                    cur_edge_r, cur_edge_l = all_edges[idx][edgedirection][0]
                    adj_edge_r, adj_edge_l = all_edges[adjacent][edgeotherdirection][0]
                    # 两个邻接窗户的横坐标
